...
class CameraViewController: UIViewController, AVCaptureFileOutputRecordingDelegate {
    ...
    //MF Edits - added countdown timer
	...
	private func configureSession() {
		...
		*/
		...
		do {
			...
			// Choose the back dual camra if available, otherwise default to a wide angle camera.
	...
	@IBAction private func toggleMovieRecording(_ recordButton: UIButton) {
		...
		*/
        ...
        if !movieFileOutput.isRecording {
            self.startCoundownTimer(Duration: duration)
        } else {
    ...
    func startCoundownTimer(Duration: Int) {
        countdownTimer.isHidden = false
        countdownTimer.text = String(Duration)
        updateCountDownTimer(Duration: (Duration))
    }
    
    func updateCountDownTimer(Duration: Int) {
        // If there is still time to wait, display next value in a second
        delay(1){
            if(Duration > 0){
                // Display next value
                self.countdownTimer.text = String(Duration-1)
                self.updateCountDownTimer(Duration: (Duration-1))
